...
void SelfCentral::onShow()
{
    ...
    QString S=QString("select * from Client where Name='%1'").arg(name);
...
void SelfCentral::on_Save_clicked()
{
    ...
    QString name = ui->Name->text();
    S1=QString("delete from Client where Name='%1'").arg(name);
    ...
    S2=QString("insert into Client (Name,Password,姓名,住址,联系方式)values('%1','%2','%3','%4','%5');").arg(item1).arg(item2).arg(item3).arg(item4).arg(item5);
