...
void OrderManagement::ReceiveAccount(QString S1, QString S2)
{
    account = S1;
    password = S2;
}
            ...
            if(object.contains("Table")){
                ...
                while(j<Row)
                {
                    for (int i = 0; i<Column; i++)
                    {
                        ...
                        a++;
                        //让发来的信息不可修改信息不可修改
                        if(ui->OrderTable->item(j,i)->text()=="购物车中")
                        {
...
void OrderManagement::AddIntoOrder(QString category, QString name)
{
    Add();
    Category=category;
    Name = name;
...
void OrderManagement::onShow()
{
    ...
    QString S=QString("select * from Ordering where 用户名='%1'").arg(account);
...
void OrderManagement::on_Search_clicked()
{
    ...
    if(s == "")
    {
        ...
        QString S = QString("select * from Ordering where 订单状况= '%1'").arg(s);
        QJsonObject jsonObject;
        jsonObject.insert("Name", "Ordering");
...
void OrderManagement::Add()
{
    ...
    S=QString("insert into Ordering (用户名,购买种类,购买名称,订单状况)values('%1','%2','%3','%4');").arg(account).arg(Category).arg(Name).arg("购物车中");
    QJsonObject jsonObject;
    jsonObject.insert("Name", "Ordering");
