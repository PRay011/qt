...
int main(int argc, char *argv[])
{
    ...
    //传递账号到各个页面（因为会用到帐号去进行多种查找操作）
    //登陆界面到注册界面
    QObject::connect(&l, SIGNAL(LoginClosed1()), &e, SLOT(EnrollOpen()));
    //登陆页面传递账号到个人中心
    QObject::connect(&l, SIGNAL(SendAccount(QString,QString)), &self, SLOT(ReceiveAccount(QString,QString)));
    //注册页面传递账号到个人中心
    QObject::connect(&e, SIGNAL(SendAccount2(QString,QString)), &self, SLOT(ReceiveAccount(QString,QString)));
    //登陆页面传递账号到订单页面
    QObject::connect(&l, SIGNAL(SendAccount(QString,QString)), &o, SLOT(ReceiveAccount(QString,QString)));
    //注册页面传递账号到订单页面
    QObject::connect(&e, SIGNAL(SendAccount2(QString,QString)), &o, SLOT(ReceiveAccount(QString,QString)));
    //登录页面传递账号到订单页面
    QObject::connect(&l, SIGNAL(SendAccount(QString,QString)), &c, SLOT(ReceiveAccount(QString,QString)));
    //注册页面传递账号到聊天界面
    QObject::connect(&e, SIGNAL(SendAccount2(QString,QString)), &c, SLOT(ReceiveAccount(QString,QString)));
    ...
    //水果界面将物品加入购物车（也就是订单页面）
    QObject::connect(&f, SIGNAL(sendCategoryAndName(QString ,QString )), &o, SLOT(AddIntoOrder(QString,QString)));
